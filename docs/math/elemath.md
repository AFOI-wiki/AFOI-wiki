...
其中 $n!$ 为 $n$ 的**阶乘**, $n!!$ 为 $n$ 的**双阶乘**, $\bmod$ 运算符 (读作 "**模**运算符") 则是我们熟悉的带余除法, 如 $15\bmod 7=1$. 不难发现双阶乘实际就是相邻的一串奇数或偶数相乘.
...
**证** 这里只证等比数列 $q\neq 1$ 的情况, 注意到 $qS_n=\sum\limits_{i=1}^nqa_i=\sum\limits_{i=2}^{n+1}a_{i}$, 与 $S_n=\sum\limits_{i=1}^na_i$ 相减即得 $(1-q)S_n=a_1(1-q^n)$. $\square$
